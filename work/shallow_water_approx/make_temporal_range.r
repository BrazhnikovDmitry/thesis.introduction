...
T <- seq(1/60, 30, len=1e3)*60

om <- 2*pi/T; H <- c(500, 1000, 1500)
...
pdf("tsunami_regimes.pdf", width=10, height=6)
par(cex.axis=1.5)
par(cex.lab=1.5)
par(mar=c(5,5,4,2)+0.1)
plot(c(min(T/60), max(T/60)+3), c(min(lambda), max(lambda)), type="n", xlab="Wave period [mins]", ylab=expression(lambda~~group("[", "km", "]")))#, xlim=c(0, 15), ylim=c(0, 200))
for (j in 1:length(H)) {
	lines(T/60, lambda[,j], col=COLS[j], lwd=2.5)
}
axis(1, at=8, labels=8)
#pointLabel(rep(1.01, length(H)), cr[1e3,], labels=paste(H, "m"), col=COLS, cex=1.5, srt=90)
text(rep(32, length(H)), lambda[1e3,], labels=paste(H, "m"), col=COLS, cex=1.5)
abline(h=50, lwd=2, lty=2)
abline(h=130, lwd=2, lty=2)
abline(v=c(5, 10, 12.5, 17.5, 20, 22.5, 27.5, 30, 35), lwd=1.2, lty=2)
